  ...
  return (
    <section id="cta" className="relative overflow-hidden" style={{ background: 'var(--bg)' }}>
      <div
        ...
        style={{ fontSize: 'clamp(100px,18vw,240px)', color: 'rgba(240,237,232,0.02)' }}
      >
        SC360
      </div>
      <div ref={ref} className="relative z-10 px-4 sm:px-6 md:px-10 py-20 sm:py-24 md:py-28 text-center">
        ...
        >
          Thousands of fitness coaches are already using SmartCoach360 to save time, keep more clients, and grow their revenue — all from one platform.
        </p>
        ...
        >
          {['Free 20-min demo', 'Explore the platform', 'See it in action'].map(t => (
...
export default function UseCases() {
  ...
  // Scroll to section when tab selected
  ...
  const visibleCases = activeId === 'all'
    ? USE_CASES
    : USE_CASES.filter(uc => uc.id === activeId)

  return (
    <div style={{ minHeight: '100vh', background: '#080808' }}>
      <main>
        <UseCaseHero />
        <UseCaseNav activeId={activeId} onSelect={handleSelect} />
        {USE_CASES.map(uc => (
          <UseCaseSection key={uc.id} {...uc} />
        ))}
        <ComparisonTable />
        <UseCaseCTA />
      </main>
